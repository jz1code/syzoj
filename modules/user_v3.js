...
app.post('/api/v3/login', async (req, res) => {
  try {
    ...
    //   res.send({success: false, error_code: 1003 });
    // } else if (user.password !== req.body.password) {
    //   res.send({success: false, error_code: 1002 });
    } else {
      token = jwt.sign(user, syzoj.config.session_secret, { expiresIn: '1d' })

      // res.set("jwt", token);

      res.send({success: true, data: user, jwt: token})
    }
  } catch(e) {
    res.send({})
  }
